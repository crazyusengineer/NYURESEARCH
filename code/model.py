...
class ThreeV2I_BC(nn.Module):
    ...
    def __init__(self,model_type=None):
        ...
        if model_type=="Bagnet33":
            ...
            self.model.conv1=torch.nn.Conv2d(in_channels=6, out_channels=64,
                               kernel_size=1, stride=1,padding=0,bias=False)
            num_ftrs = self.model.fc.in_features
            self.model.fc=torch.nn.Linear(num_ftrs, 1)
        if model_type=="vgg16":
            self.model = models.vgg16(pretrained=False)
            self.model.features[0]=torch.nn.Conv2d(in_channels=6, out_channels=64,
...
class ThreeV2I_ML(nn.Module):
    ...
    def __init__(self, model_type=None):
        ...
        if model_type == "vgg16":
            ...
            self.model_3V.features[0] = torch.nn.Conv2d(in_channels=3, out_channels=64,
                                                        kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        ...
        if model_type == "Bagnet33":
            ...
            self.model_3V.conv1 = torch.nn.Conv2d(in_channels=3, out_channels=64,
                                                  kernel_size=1, stride=1, padding=0, bias=False)
